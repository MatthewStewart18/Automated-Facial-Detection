...
indicesLabel1 = find(preprocessinglabels == 1);
indicesLabelMinus1 = find(preprocessinglabels == -1);

% Determine the number of columns (max images in a row)
maxPerRowFace = length(indicesLabel1);
rowNonFace = length(indicesLabelMinus1);
numRows = 1; % Two rows: one for each label

% Initialize figure
figure;
sgtitle('Face Images After Augmentation');

% Display images with label 1 in the top row
for i = 1:length(indicesLabel1)
    subplot(numRows, maxPerRow, i); % First row
    imshow(preprocessingimages{indicesLabel1(i)}, []);
    title(['Label 1']);
end
figure;
sgtitle('Non-Face Images After Augmentation');
% Display images with label -1 in the bottom row
for i = 1:length(indicesLabelMinus1)
    subplot(numRows, rowNonFace, i); % Second row
